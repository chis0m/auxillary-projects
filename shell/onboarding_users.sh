...
# These keys were intentionally exposed. They are just test keys for practice
# PLEASE IGNORE

# create aux project private
cat > ~/.ssh/aux_proj <<EOL
...
# create aux project public key
cat > ~/.ssh/aux_proj.pub <<EOL
...
group=developers                                            # Define variables
#password=test
...
function creatUser() {                                      # define function to abstract user creation logic
    if grep -q "^${1}:" /etc/passwd; then                  # checks if user already exists
        ...
        cat ~/.ssh/aux_proj.pub >> ~/.ssh/authorized_keys   # append aux proj public key to auth key file
